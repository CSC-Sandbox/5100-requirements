...
public class TestDisplayAffect {
  ...
  private static void sendAffect(PrintWriter out) throws IOException {
    ...
    while (!out.checkError()) {
      double t = sample * .10;
      double focus = wave(t, 0, .55, .35), excitement = wave(t, 1.2, .45, .30), engagement = wave(t * .70, 2.1, .65, .25), interest = wave(t * .85, 3, .55, .30), stress = wave(t * .55, 4, .35, .25);
      out.printf(Locale.US, "AFFECT,%.2f,%.2f,%.2f,%.2f,%.2f%n", focus, excitement, engagement, interest, stress);
      out.flush();
      sample++;
      sleep();
    }
  ...
  private static double wave(double t, double phase, double center, double amp) {
    double v = center + amp * Math.sin(t + phase);
